...
def test_asset_version_and_attachment_cascade(session):
    ...
    version.attachments.append(Attachment(kind="model", uri="file://cube-v1.stl", checksum="abc123"))
...
def test_asset_tag_association(session):
    ...
    with pytest.raises(IntegrityError):
        session.execute(asset_tag_table.insert().values(asset_id=asset.id, tag_id=material.id))
    session.rollback()
...
def test_asset_relationship_metadata(session):
    ...
    parent_sources = {rel.source_asset.name for rel in stored_carriage.incoming_relationships}
