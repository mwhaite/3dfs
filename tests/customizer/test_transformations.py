...
try:  # pragma: no cover - exercised when build123d is unavailable
    ...
    pytest.skip("build123d is required for transformation tests", allow_module_level=True)
...
def test_emboss_mesh_records_component_metadata(base_mesh_path: Path, tmp_path: Path) -> None:
    with BuildPart() as part:
        Cylinder(radius=0.2, height=0.6, align=(Align.CENTER, Align.CENTER, Align.CENTER))
