...
* :class:`AssetRelationship` – directed links between assets used to model
  containers, related structures, and other metadata-rich associations.
...
class Version(Base):
    ...
    __table_args__ = (UniqueConstraint("asset_id", "number", name="uq_version_number_per_asset"),)
    ...
    printer_profile: Mapped[PrinterProfile | None] = relationship(back_populates="versions")
...
class Attachment(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    version_id: Mapped[int] = mapped_column(ForeignKey("versions.id", ondelete="CASCADE"), index=True, nullable=False)
