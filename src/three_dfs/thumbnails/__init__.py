...
class ThumbnailRenderer:
        ...
        *,
        ...
        if mesh_data is None:
            raise ThumbnailGenerationError(
                f"No renderable mesh data found for {source_path!s}"
            )
    ...
    def _render_mesh(
        ...
        rotation = _compose_rotation_matrix(
            np.deg2rad(25.0), np.deg2rad(-35.0), np.deg2rad(30.0)
        )
...
class ThumbnailCache:
    ...
    def _hash_source(self, source_path: Path) -> str:
        ...
        try:
            ...
            raise ThumbnailGenerationError(
                f"Unable to read {source_path!s}: {exc}"
            ) from exc
    ...
    def _resolve_source_path(self, asset: AssetRecord) -> Path:
        ...
        for candidate in candidates:
            try:
                ...
                logger.debug(
                    "Invalid path candidate %s for %s: %s", candidate, asset, exc
                )
                continue
        ...
        raise ThumbnailGenerationError(
            f"No readable source file found for asset {asset.path!s}"
        )
