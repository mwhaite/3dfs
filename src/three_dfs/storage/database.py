...
-- Containers: collections of assets/components
...
CREATE TABLE IF NOT EXISTS container_components (
    ...
    metadata TEXT NOT NULL DEFAULT '{}',
    UNIQUE(container_id, asset_id, order_index)
);

CREATE INDEX IF NOT EXISTS idx_container_components_container_id
    ON container_components(container_id);
CREATE INDEX IF NOT EXISTS idx_container_components_asset_id
    ON container_components(asset_id);

CREATE TABLE IF NOT EXISTS container_versions (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    container_asset_id INTEGER NOT NULL REFERENCES assets(id) ON DELETE CASCADE,
    name TEXT NOT NULL,
    metadata TEXT NOT NULL DEFAULT '{}',
    notes TEXT,
    source_version_id INTEGER REFERENCES container_versions(id) ON DELETE SET NULL,
    created_at TEXT NOT NULL,
    UNIQUE(container_asset_id, name)
);

CREATE INDEX IF NOT EXISTS idx_container_versions_container_asset_id
    ON container_versions(container_asset_id);
