...
class AssetRepository:
    ...
    def list_assets(self) -> list[AssetRecord]:
        ...
        with self._storage.connect() as connection:
            rows = connection.execute("SELECT * FROM assets ORDER BY path COLLATE NOCASE").fetchall()
    ...
    def list_customizations_for_asset(self, base_asset_id: int) -> list[CustomizationRecord]:
    ...
    def list_derivatives_for_asset(
        ...
        *,
        ...
        logger.info(f"Listing derivatives for asset: {base_asset_id}")
        ...
        with self._storage.connect() as connection:
            rows = connection.execute(query, params).fetchall()
            logger.info(f"Found {len(rows)} derivatives")
            return self._rows_to_records(connection, rows)
    ...
    def create_container_version(
        ...
        *,
        ...
        """Persist a snapshot of a container asset."""

        normalized_name = self._normalize_version_name(name)
        serialized_metadata = self._serialize_metadata(metadata)
        normalized_notes = self._normalize_optional_text(notes)
        now = datetime.now(UTC)

        with self._storage.connect() as connection:
            validated_source_id: int | None = None
            if source_version_id is not None:
                source_row = self._fetch_container_version_row(
                    connection,
                    source_version_id,
                )
                if source_row is None:
                    raise KeyError(f"Container version {source_version_id} does not exist")
                if int(source_row["container_asset_id"]) != container_asset_id:
                    raise ValueError("Source version does not belong to the same container")
                validated_source_id = source_version_id
            ...
            row = self._fetch_container_version_row(connection, version_id)

        if row is None:  # pragma: no cover - defensive safeguard
            raise RuntimeError("Failed to create container version")
        return self._row_to_container_version_record(row)

    def list_container_versions(self, container_asset_id: int) -> list[ContainerVersionRecord]:
        """Return all versions recorded for *container_asset_id*."""

        with self._storage.connect() as connection:
            rows = connection.execute(
                """
                SELECT * FROM container_versions
                WHERE container_asset_id = ?
                ORDER BY created_at DESC, id DESC
                """,
                (container_asset_id,),
            ).fetchall()

        return [self._row_to_container_version_record(row) for row in rows]
    ...
    def get_container_version_by_name(self, container_asset_id: int, name: str) -> ContainerVersionRecord | None:
        """Return the version identified by *name* for a container."""
        ...
        with self._storage.connect() as connection:
            row = connection.execute(
                """
                SELECT * FROM container_versions
                WHERE container_asset_id = ? AND name = ?
                LIMIT 1
                """,
                (container_asset_id, normalized_name),
            ).fetchone()
        if row is None:
            return None
        return self._row_to_container_version_record(row)
    ...
    def get_latest_container_version(self, container_asset_id: int) -> ContainerVersionRecord | None:
        """Return the most recent version for the given container."""
    ...
    def delete_container_version(self, version_id: int) -> bool:
        """Delete a container version by identifier."""

        with self._storage.connect() as connection:
            cursor = connection.execute(
                "DELETE FROM container_versions WHERE id = ?",
                (version_id,),
            )
            return cursor.rowcount > 0

    def rename_container_version(
        self,
        version_id: int,
        *,
        name: str,
        ...
        with self._storage.connect() as connection:
            row = self._fetch_container_version_row(connection, version_id)
            if row is None:
                raise KeyError(f"Container version {version_id} does not exist")
            try:
                connection.execute(
                    "UPDATE container_versions SET name = ? WHERE id = ?",
                    (normalized_name, version_id),
                )
            except IntegrityError as exc:
                raise ValueError("Version name must be unique per container") from exc
            row = self._fetch_container_version_row(connection, version_id)
        if row is None:  # pragma: no cover
            raise RuntimeError("Failed to rename container version")
        return self._row_to_container_version_record(row)
    ...
    def paths_for_tag(self, tag: str) -> list[str]:
        ...
        with self._storage.connect() as connection:
            rows = connection.execute(
                """
                SELECT assets.path AS path
                FROM assets
                JOIN asset_tag_links ON asset_tag_links.asset_id = assets.id
                JOIN tags ON tags.id = asset_tag_links.tag_id
                WHERE tags.name = ? COLLATE NOCASE
                ORDER BY assets.path COLLATE NOCASE
                """,
                (normalized,),
            ).fetchall()
    ...
    def _normalize_version_name(self, name: str) -> str:
    ...
    def _fetch_asset_relationship_row(self, connection: Connection, relationship_id: int):
        return connection.execute(
            "SELECT * FROM asset_relationships WHERE id = ?",
            (relationship_id,),
        ).fetchone()

    def _fetch_container_version_row(self, connection: Connection, version_id: int):
        return connection.execute(
            "SELECT * FROM container_versions WHERE id = ?",
            (version_id,),
        ).fetchone()
    ...
    def _tags_for_asset_ids(self, connection: Connection, asset_ids: list[int]) -> dict[int, list[str]]:
        if not asset_ids:
            return {}
    ...
    def _row_to_container_version_record(self, row) -> ContainerVersionRecord:
        created_at = datetime.fromisoformat(row["created_at"])
        metadata = self._deserialize_metadata(row["metadata"])
        notes = row["notes"]
        normalized_notes = str(notes).strip() if notes is not None else None
        source_version_id = row["source_version_id"]
        try:
            source_id = int(source_version_id) if source_version_id is not None else None
        except Exception:
            source_id = None
        return ContainerVersionRecord(
            id=row["id"],
            container_asset_id=row["container_asset_id"],
            name=str(row["name"]),
            metadata=metadata,
