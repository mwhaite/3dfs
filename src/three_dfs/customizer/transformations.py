...
def _create_text_shape(
    ...
    *,
    ...
    if gp_Vec is None or BRepPrimAPI_MakePrism is None or Solid is None:
        raise TransformationError("Text extrusion requires OCC primitives")

    vec = gp_Vec(extrusion_direction.X, extrusion_direction.Y, extrusion_direction.Z)
    extrusions = []
    for face in faces:
        prism = BRepPrimAPI_MakePrism(face.wrapped, vec, True)
        extrusions.append(Solid(prism.Shape()))
...
class TransformationDescriptor(ABC):
    ...
    @classmethod
    def from_dict(cls: type[_DescriptorType], data: Mapping[str, Any]) -> _DescriptorType:
    ...
    @classmethod
    @abstractmethod
    def _from_dict(cls: type[_DescriptorType], data: Mapping[str, Any]) -> _DescriptorType:
        raise NotImplementedError

    @abstractmethod
    def apply(self, shape: Shape, context: _TransformationContext) -> tuple[Shape, dict[str, Any]]:
...
class ScaleTransformation(TransformationDescriptor):
    ...
    def apply(self, shape: Shape, context: _TransformationContext) -> tuple[Shape, dict[str, Any]]:
        ...
        if self.origin is not None:
            ...
            transformed = _apply_matrix(transformed, _translation_matrix(inverse_origin))
...
def _export_shape(
    ...
    if suffix != ".stl":
        raise TransformationError(f"Unsupported export format '{suffix or 'unknown'}'; only STL is supported")
...
def apply_transformations(
    ...
    *,
    ...
    for index, descriptor in enumerate(_normalise_descriptors(transformations), start=1):
        current_shape, metadata = descriptor.apply(current_shape, context)
        module_definition, module_name = descriptor.openscad_module(previous_module, index)
