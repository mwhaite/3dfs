...
GCODE_EXTENSIONS: Final[frozenset[str]] = frozenset(
    {
        ".gcode",
        ".gco",
        ".g",
...
"""Recognised G-code program suffixes."""
...
def _import_local_asset(
    ...
    *,
    ...
    if extension not in SUPPORTED_EXTENSIONS and extension not in GCODE_EXTENSIONS:
        raise UnsupportedAssetTypeError(f"Unsupported asset format '{extension or 'unknown'}'")
...
def _import_remote_asset(
    ...
    *,
    ...
    managed_root.mkdir(parents=True, exist_ok=True)
    destination = _allocate_destination(managed_root, _derive_destination_name(identifier))

    try:
        plugin_metadata_raw = plugin.fetch(identifier, destination)
    except Exception as exc:  # pragma: no cover - defensive safety net
        destination.unlink(missing_ok=True)
        raise AssetImportError(f"Import plugin {plugin.__class__.__name__} failed to fetch {identifier}") from exc

    try:
        plugin_metadata = dict(plugin_metadata_raw or {})
    except TypeError as exc:
        destination.unlink(missing_ok=True)
        raise AssetImportError(f"Import plugin {plugin.__class__.__name__} returned invalid metadata") from exc
    ...
    if not final_path.exists():
        raise AssetImportError(
            "Import plugin " f"{plugin.__class__.__name__} did not materialize an asset for {identifier}"
        )

    if not final_path.is_file():
        raise AssetImportError(
            "Import plugin " f"{plugin.__class__.__name__} produced a non-file destination {final_path}"
        )
    ...
    if extension not in SUPPORTED_EXTENSIONS and extension not in GCODE_EXTENSIONS:
        ...
        raise UnsupportedAssetTypeError("Unsupported asset format " f"'{extension or 'unknown'}' from import plugin")
